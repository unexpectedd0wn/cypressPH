...
let wholesaler = Wholesalers.PCO.Name;
let orderMinValue = piMinOrderValue.PCO;
            ...
            cy.wait('@orderHistory').then(({ response }) => {
                ...
                cy.get(response.body.items).each(($item, index) => {
                    ...
                    if ($item.orderRef == Cypress.env('orderId')) {
                        cy.log("Found order!")
                        let updatedPriceValue = Cypress.env('item.NetPrice') * (updateQty + 1);
