...
describe('Manual Orders', () => {
    ...
    function randomItem() { // min and max included 
     ...
     before(() => {
        ...
        cy.getItemForTest(Wholeslaers.IMED.Name)
    ...
    it.only('Order IMED Item', () => {
       ...
       cy.wait('@itemAdded').then(({ response }) => {
        ...
        shoppingCart.state_UDShoppingCart(Wholeslaers.IMED.Name,
            '',
        ...
        let updateQty = Math.round((100 / Cypress.env('item.NetPrice')))
        ...
        cy.wait('@sendorder').then(({ response }) => {
            ...
            shoppingCart.successfulOrderToastMessage(Wholeslaers.IMED.Name)
                ...
                cy.get(response.body.items).each(($item, index) => {
                    ...
                    if ($item.orderRef == Cypress.env('orderId')) {
                        ...
                        .should(`deep.equal`, {
                            ...
                            wholeslaer: Wholeslaers.IMED.Name,
                            numberOfItem: '1',
